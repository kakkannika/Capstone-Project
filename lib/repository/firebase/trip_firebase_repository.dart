...
import 'package:tourism_app/repository/trip_repository.dart';
...
class TripFirebaseRepository extends TripRepository {
  ...
  Future<List<Day>> _fetchDaysForTrip(
      firestore.DocumentReference tripRef) async {
    try {
      final daysSnapshot =
          await tripRef.collection('days').orderBy('dayNumber').get();
  ...
  Future<List<Place>> _fetchPlacesFromMainCollection(
  ...
  Future<void> addPlaceToDay({
    ...
    try {
      ...
      final currentPlaceIds =
          List<String>.from(dayDoc.data()?['placeIds'] ?? []);
    ...
    try {
      ...
      if (endDate != null) {
        final currentEndDate =
            (tripDoc.data()?['endDate'] as firestore.Timestamp).toDate();
        final currentStartDate =
            (tripDoc.data()?['startDate'] as firestore.Timestamp).toDate();
        // If the new end date is later than the current one
        if (endDate.isAfter(currentEndDate)) {
          // Calculate the current number of days
          final currentNumberOfDays =
              currentEndDate.difference(currentStartDate).inDays + 1;
          // Calculate the new number of days
          final newNumberOfDays =
              endDate.difference(startDate ?? currentStartDate).inDays + 1;
