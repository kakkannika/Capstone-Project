...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
        ...
        if (trip != null && mounted) {
          setState(() {
            ...
            if (widget.isEditing &&
              ...
              final expenseDate = DateTime(widget.expense!.date.year,
                  widget.expense!.date.month, widget.expense!.date.day);
              final tripStartDate = DateTime(trip.startDate.year,
              ...
              if (difference >= 0 && difference < trip.days.length) {
                ...
                _selectedDate =
                    trip.startDate.add(Duration(days: _selectedDayIndex));
              }
  ...
  void _updateAvailableBudget() {
    ...
    setState(() {
      ...
      if (_availableBudgetForSelectedDay <= 0 &&
          budget.total > 0 &&
          _trip!.days.isNotEmpty) {
        _availableBudgetForSelectedDay = budgetProvider.calculateDailyBudget(
            budget.total, _trip!.days.length);
      }
    });
  }
  ...
  String _formatDayForDropdown(int dayIndex) {
    ...
    String formattedDate =
        "${DateFormat('EEE').format(date)} ${DateFormat('dd/MM').format(date)}";
  ...
  Future<void> _addExpense() async {
    ...
    if (!widget.isEditing) {
      ...
      isOverBudget = _totalExpense > _availableBudgetForSelectedDay ||
          _totalExpense > widget.remainingBudget;
    ...
    try {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                "Failed to ${widget.isEditing ? 'update' : 'add'} expense. Please try again."),
            backgroundColor: DertamColors.red,
          ),
        );
      }
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<int>(
              ...
              onChanged: (int? newValue) {
                if (newValue != null) {
                  DateTime newDate =
                      _trip!.startDate.add(Duration(days: newValue));
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 12,
                  color: _availableBudgetForSelectedDay > 0
                      ? DertamColors.green
                      : DertamColors.red,
                  fontWeight: FontWeight.bold,
                ),
